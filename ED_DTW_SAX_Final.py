...
for i, (dataset, warping_window) in enumerate(zip(dataset_list, warping_window_list)):
    ...
    clf_weasel = LogisticRegression(penalty='l2', C=1, fit_intercept=True,solver='liblinear', multi_class='ovr')
...
er_final = [error_ed,error_dtw,error_dtw_w,error_SAX,error_boss,1-accuracy_BOSSVS,1-accuracy_weasel]
acc_final = [1-error_ed,1-error_dtw,1-error_dtw_w,1-error_SAX,1-error_boss,accuracy_BOSSVS,accuracy_weasel]
...
sheet1.write(0,4,'BOSS error rate')
sheet1.write(0,5,'SAX error rate')
sheet1.write(0,6,'BOSSVS error rate')
sheet1.write(0,7,'WEASEL error rate')
sheet1.write(0,8,'Minimum Error rate')
sheet1.write(0,9,'Maximum Accuracy')
...
sheet1.write(int(nor),4, float(error_boss))
sheet1.write(int(nor),5, float(error_SAX))
sheet1.write(int(nor),6, float(1-accuracy_BOSSVS))
sheet1.write(int(nor),7, float(1-accuracy_weasel))
sheet1.write(int(nor),8, float(min(er_final)))
sheet1.write(int(nor),9, float(max(acc_final)))
